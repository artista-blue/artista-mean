...
function generateSchema (model) {
	...
	schemaData = model.types,
	lines = [],
	schemaLines = [];
    lines.push("");
    lines.push(util.format("const %sSchema = mongoose.Schema({", modelName));

    Object.keys(schemaData).forEach(function (name) {
	const type = schemaData[name];
 ...
 */
MongooseUtils.init = function (projectName, modelList, tmplPath, outDir, callback) {
    var db = projectName,
	template;
    async.waterfall([
